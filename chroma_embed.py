...
from eda.preprocess import convert_to_indices, pad_truncate
from eda.two_tower_architecture import TwoTowerModel

DATA_PATH = "./train_triples_v1.1.json"
MODEL_PATH = "./model_twotower1.pth"
VOCAB_PATH = "./word_to_ids.pkl"
...
class Embeddings:
    ...
    def embeddings(self, tokens: List[str]) -> np.ndarray:
        ...
        _, documents_embed, _ = model(
            tokens, tokens, tokens, query_lengths, doc_lengths
        )
...
if __name__ == "__main__":
    ...
    for i, dd in tqdm(enumerate(train)):
        doc = dd["positive"]
        documents.append(
            Document(page_content=doc, metadata={"source": "search"}, id=i)
        )
